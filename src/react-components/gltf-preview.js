...
class GLTFPreview extends Component {
  static propTypes = {
    gltfGltfUrl: PropTypes.string,
  ...
  componentDidMount = () => {
    ...
    if (this.props.gltfGltfUrl) {
      this.loadPreviewGLTF(this.props.gltfGltfUrl).then(this.setGLTF);
    }
  ...
  resetCamera = (() => {
    ...
    return () => {
      box.setFromObject(this.gltf);
      box.getCenter(center);

      // Shift the center vertically in order to frame the gltf nicely.
      //center.y = (box.max.y - box.min.y) * 0.6 + box.min.y;

      fitBoxInFrustum(this.camera, box, center);
      fitBoxInFrustum(this.snapshotCamera, box, center, 1.3);
  ...
  componentWillUnmount = () => {
    // Gotta be particularly careful about disposing things here since we will likely create many gltf
  ...
  componentDidUpdate = async oldProps => {
    if (oldProps.gltfGltfUrl !== this.props.gltfGltfUrl) {
      if (this.gltf) {
      ...
      if (this.props.gltfGltfUrl) {
        ...
        await this.loadPreviewGLTF(this.props.gltfGltfUrl).then(this.setGLTF);
      }
    }
        ...
        {this.props.gltfGltfUrl &&
          (this.state.error && !this.state.loading) && (
            <div className="error">
              ...
              />
              <FormattedMessage id="gltf-preview.loading-failed" />
            </div>
          )}
...
export default injectIntl(GLTFPreview, { withRef: true });
